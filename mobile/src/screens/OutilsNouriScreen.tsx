...
import AddOutilNouriFab from '../components/outilsNouri/AddOutilNouriFab'
import OutilNouriFormModal from '../components/outilsNouri/OutilNouriFormModal'
import OutilNouriListItem from '../components/outilsNouri/OutilNouriListItem'
import OutilNouriSkeleton from '../components/outilsNouri/OutilNouriSkeleton'
import AppToast from '../components/ui/AppToast'
import { fetchOutilsNouri } from '../lib/outilNouriApi'
import { formatMontant } from '../lib/formatMoney'
import { theme } from '../theme/appTheme'
import type { OutilNouri } from '../types/outilNouri'
...
export default function OutilsNouriScreen({
  accessToken,
  canViewEquipeOutils,
  drawerOpen = false,
}: Props) {
  const [entries, setEntries] = useState<OutilNouri[]>([])
  const [loading, setLoading] = useState(true)
  const [refreshing, setRefreshing] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [search, setSearch] = useState('')
  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<OutilNouri | null>(null)
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const list = await fetchOutilsNouri(accessToken)
      setEntries(list)
    } catch (e) {
      setEntries([])
  ...
  const totalPrixNouri = useMemo(
    () => filtered.reduce((s, o) => s + o.prixNouri, 0),
    [filtered]
  )
  ...
  const openEdit = (entry: OutilNouri) => {
    setEditing(entry)
    setShowForm(true)
  }
  ...
  if (!canViewEquipeOutils) {
    return (
      <View style={styles.denied}>
        ...
        <Text style={styles.deniedSub}>Vous n&apos;avez pas accès à l&apos;Opération Nouri.</Text>
      </View>
    )
  }
      ...
      <View style={styles.heroCard}>
        <View style={styles.heroTop}>
          ...
          <View style={styles.heroText}>
            <Text style={styles.heroTitle}>Opération Nouri</Text>
            <Text style={styles.heroSub}>Suivi travaux, prix garage & Nouri</Text>
          </View>
          {entries.length > 0 ? (
        ...
        <LinearGradient
          colors={totalPrixNouri >= 0 ? ['#f0f9ff', '#e0f2fe'] : ['#fffbeb', '#ffedd5']}
        ...
        >
          <View style={styles.kpiIcon}>
            <Ionicons
              name="wallet-outline"
              size={20}
              color={totalPrixNouri >= 0 ? ACCENT : '#d97706'}
            />
          </View>
          <View style={styles.kpiText}>
            <Text style={styles.kpiLabel}>Solde Prix Nouri</Text>
            <Text
              style={[
                styles.kpiValue,
                { color: totalPrixNouri >= 0 ? '#0369a1' : '#b45309' },
              ]}
            >
              {formatMontant(totalPrixNouri)} TND
            </Text>
            {search.trim() ? (
  ...
  return (
    <View style={styles.root}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <OutilNouriListItem entry={item} onPress={() => openEdit(item)} />
        )}
      ...
      <AddOutilNouriFab onPress={openNew} visible={!overlayOpen} />
...
const styles = StyleSheet.create({
  ...
  countBadgeText: { fontSize: 14, fontWeight: '800', color: '#0369a1' },
  kpiBanner: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
