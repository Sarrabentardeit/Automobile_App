...
import { fetchFacturesVente, updateFactureStatut } from '../lib/factureVenteApi'
import { factureResteTTC, factureTotalTTC } from '../lib/factureVenteHelpers'
...
export default function PaiementsVenteScreen({
  accessToken,
  canViewFinance,
  drawerOpen = false,
}: Props) {
  ...
  const stats = useMemo(() => {
    const resteTotal = factures.reduce((s, f) => s + factureResteTTC(f), 0)
    const payeTotal = factures.reduce(
      (s, f) => s + factureTotalTTC(f.lignes, f.timbre) - factureResteTTC(f),
      0
    )
            ...
            <View style={styles.empty}>
              ...
              <Text style={styles.emptySub}>
                {searchDebounced
                  ...
                  : 'Aucune facture partiellement payée en cours.'}
              </Text>
            </View>
          )
        }
