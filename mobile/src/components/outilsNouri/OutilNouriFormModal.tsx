...
function emptyForm(): OutilNouriInput {
  return {
    date: todayIsoDate(),
    vehicule: '',
    typeTravaux: '',
...
export default function OutilNouriFormModal({
  visible,
  entry,
  accessToken,
  onClose,
  ...
  const isEdit = !!entry
  const [form, setForm] = useState<OutilNouriInput>(emptyForm)
  const [prixGarageText, setPrixGarageText] = useState('')
  const [prixNouriText, setPrixNouriText] = useState('0')
  ...
  useEffect(() => {
    ...
    if (entry) {
      setForm({
        ...
        prixNouri: entry.prixNouri,
      })
      setPrixGarageText(
        entry.prixGarage != null && entry.prixGarage !== 0 ? String(entry.prixGarage) : ''
      )
      setPrixNouriText(String(entry.prixNouri))
    } else {
      setForm(emptyForm())
      setPrixGarageText('')
      setPrixNouriText('0')
  ...
  const canSave = form.date.trim().length > 0 && prixNouriText.trim().length > 0
      ...
      { text: 'Annuler', style: 'cancel' },
      {
        ...
        onPress: () => {
          void (async () => {
            setSaving(true)
            try {
              await deleteOutilNouri(accessToken, entry.id)
              onDeleted()
              onClose()
            } catch (e) {
  ...
  const submit = async () => {
    if (!canSave) return
    const prixNouri = parseAmount(prixNouriText)
    if (prixNouri === undefined) {
      setError('Prix Nouri invalide')
      return
    }
    setError(null)
    const payload: OutilNouriInput = {
      date: form.date.trim(),
    ...
    try {
      if (isEdit && entry) {
        await updateOutilNouri(accessToken, entry.id, payload)
      } else {
        await createOutilNouri(accessToken, payload)
      }
      onSaved()
      onClose()
    } catch (e) {
      ...
      <View style={[styles.card, { maxHeight: cardMaxHeight }]}>
        ...
        <View style={styles.header}>
          <View style={styles.headerText}>
            ...
            <Text style={styles.subtitle}>Opération Nouri — travaux & prix</Text>
          </View>
        ...
        >
          ...
          <Field
            icon="cash-outline"
            value={prixNouriText}
            onChangeText={setPrixNouriText}
            placeholder="Prix Nouri (+revenu / -paiement) *"
            keyboardType="decimal-pad"
          />
          <Text style={styles.hint}>
            Prix Nouri positif = revenu · négatif = paiement ou avance
          </Text>
        </ScrollView>
