...
export default function OutilNouriListItem({ entry, onPress }: Props) {
  const positive = entry.prixNouri >= 0
      ...
      <View style={styles.body}>
        ...
        <View style={styles.info}>
          <View style={styles.topRow}>
            ...
            <Text style={[styles.prixNouri, positive ? styles.positive : styles.negative]}>
              {formatMontant(entry.prixNouri)} TND
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  prixNouri: { fontSize: 14, fontWeight: '800' },
  positive: { color: '#0369a1' },
  ...
  metaText: { fontSize: 11, fontWeight: '600', color: '#0369a1' },
  garageChip: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
