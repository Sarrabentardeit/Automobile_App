...
type Props = {
  ...
  accessToken: string
  onClose: () => void
  onSaved: (f: FactureVente) => void
  onError: (msg: string) => void
}

export default function FactureVentePaiementModal({
  visible,
  facture,
  accessToken,
  onClose,
  ...
  const submit = async () => {
    ...
    setSaving(true)
    try {
      const updated = await addPaiementFacture(accessToken, facture.id, {
        date: date.trim(),
        montant: m,
        mode,
        note,
      ...
      onError(e instanceof Error ? e.message : 'Erreur encaissement')
    } finally {
      setSaving(false)
    }
  }
        ...
        <View style={styles.header}>
          <View style={styles.headerText}>
            ...
            <Text style={styles.subtitle} numberOfLines={1}>
              {facture.numero} · {facture.clientNom}
            </Text>
          </View>
