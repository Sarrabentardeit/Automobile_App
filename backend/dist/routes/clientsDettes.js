...
if (typeof db.clientDette === 'undefined') {
    console.error('[clientsDettes] Le modèle Prisma ClientDette est absent. Exécutez "npx prisma generate" (avec le serveur backend arrêté), puis redémarrez le serveur.');
}
function toClientAvecDette(c) {
    return {
        id: c.id,
...
router.get('/', (0, auth_1.authenticate)(), async (req, res) => {
    try {
        ...
        const where = q
            ? {
                OR: [
                    ...
                    { telephone_client: { contains: q, mode: 'insensitive' } },
        ...
        console.error(err);
        const message = db.clientDette === undefined
...
router.get('/:id', (0, auth_1.authenticate)(), async (req, res) => {
    try {
        ...
        const c = (await db.clientDette.findUnique({ where: { id } }));
        if (!c)
            return res.status(404).json({ error: 'Client avec dette introuvable' });
