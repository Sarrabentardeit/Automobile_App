...
const CHECKLIST_ROLES = ['chef_atelier', 'coordinateur', 'technicien'];
function resetAllTodo(data) {
    return {
        version: 1,
...
async function loadRawTemplateStore() {
...
async function loadTemplateStore() {
    ...
    for (const r of CHECKLIST_ROLES) {
        ...
        if (part) {
            ...
            if (normalized.sections.length > 0)
                out[r] = resetAllTodo(normalized);
        }
    }
    return Object.keys(out).length ? out : null;
}
function effectiveTemplateForRole(role, store) {
    const custom = store?.[role];
    if (custom && custom.sections.length > 0)
        return custom;
    return createTemplate(role);
}
function validateAdminTemplatePayload(data, role) {
    const normalized = normalizeData(data ?? {}, role);
    if (normalized.sections.length === 0)
        return null;
    if (normalized.sections.length > 25)
        return null;
    let totalItems = 0;
    for (const s of normalized.sections) {
        if (!String(s.title ?? '').trim())
            return null;
        if (s.items.length === 0 || s.items.length > 80)
            return null;
        for (const it of s.items) {
            if (!String(it.label ?? '').trim())
                return null;
            if (it.label.length > 600)
                return null;
            totalItems += 1;
        }
    }
...
function toChecklistRoleKey(role) {
    const r = String(role ?? '').toLowerCase();
    if (r === 'chef_atelier' || r === 'coordinateur' || r === 'technicien')
        return r;
    return 'technicien';
}
function checklistsDataEqual(a, b) {
 ...
 */
function reconcileSavedWithTemplate(saved, template) {
    ...
    const sections = template.sections.map(ts => {
        ...
        const oldItemsById = oldSec ? new Map(oldSec.items.map(i => [i.id, i])) : new Map();
        const items = ts.items.map(ti => {
            const old = oldItemsById.get(ti.id);
            let status = 'todo';
            if (old && (old.status === 'done' || old.status === 'na' || old.status === 'todo'))
                status = old.status;
...
async function syncChecklistRowWithEffectiveTemplate(checklist, templateStore) {
    ...
    if (checklistsDataEqual(saved, reconciled))
        return checklist;
...
router.get('/admin/templates', async (req, res) => {
    try {
        if (!ensureChecklistModel(res))
            return;
        if (!isChecklistAdmin(req))
            return res.status(403).json({ error: 'Réservé aux administrateurs' });
        const store = await loadTemplateStore();
        const effective = Object.fromEntries(CHECKLIST_ROLES.map(r => [r, effectiveTemplateForRole(r, store)]));
        const defaults = Object.fromEntries(CHECKLIST_ROLES.map(r => [r, createTemplate(r)]));
        const usingCustom = Object.fromEntries(CHECKLIST_ROLES.map(r => [r, Boolean(store?.[r])]));
...
router.put('/admin/templates', async (req, res) => {
    try {
        ...
        const src = t;
        const keysToUpdate = CHECKLIST_ROLES.filter(r => Object.prototype.hasOwnProperty.call(src, r) && src[r] !== undefined && src[r] !== null);
        ...
        for (const r of keysToUpdate) {
            ...
            if (!v)
                return res.status(400).json({ error: `Modèle invalide pour le profil: ${r}` });
            validated[r] = v;
        }
        const existingRow = await db.appSetting.findUnique({ where: { key: CHECKLIST_TEMPLATES_KEY } });
        const base = existingRow?.value && typeof existingRow.value === 'object' && !Array.isArray(existingRow.value)
            ? { ...existingRow.value }
            : {};
        for (const r of keysToUpdate) {
            base[r] = validated[r];
        ...
        await db.appSetting.upsert({
            ...
            create: { key: CHECKLIST_TEMPLATES_KEY, value: base },
            update: { value: base },
        });
        const store = await loadTemplateStore();
        const effective = Object.fromEntries(CHECKLIST_ROLES.map(r => [r, effectiveTemplateForRole(r, store)]));
...
router.delete('/admin/templates/:role', async (req, res) => {
    try {
        if (!ensureChecklistModel(res))
            return;
        if (!isChecklistAdmin(req))
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const templateStore = await loadTemplateStore();
        const template = effectiveTemplateForRole(roleKey, templateStore);
        const clientData = normalizeData(req.body.data, roleKey);
