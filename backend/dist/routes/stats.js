...
function buildBase(year, groupBy) {
    if (groupBy === 'quarter') {
...
function bucketIndex(dateStr, year, groupBy) {
    if (!dateStr)
        return null;
    const [y, m] = String(dateStr).slice(0, 10).split('-').map(Number);
    if (!Number.isFinite(y) || !Number.isFinite(m) || y !== year || m < 1 || m > 12)
        return null;
    if (groupBy === 'quarter')
        return Math.floor((m - 1) / 3);
    return m - 1;
}
function factureTotalTtc(lignes, timbre) {
    let totalHt = 0;
    let depenses = 0;
    for (const l of lignes) {
        if (l.type === 'main_oeuvre')
            totalHt += (Number(l.qte) || 0) * (Number(l.mt_ht) || 0);
        else if (l.type === 'produit')
            totalHt += (Number(l.qte) || 0) * (Number(l.prix_unitaire_ht) || 0);
        else if (l.type === 'depense')
            depenses += Number(l.montant) || 0;
    }
    const tva = totalHt * 0.19;
    return totalHt + tva + depenses + (Number(timbre) || 0);
}
router.get('/trends', (0, auth_1.authenticate)(), async (req, res) => {
    try {
        const now = new Date();
        const year = Math.max(2000, Math.min(2100, Number(req.query.year) || now.getFullYear()));
        const groupBy = String(req.query.groupBy ?? 'month') === 'quarter' ? 'quarter' : 'month';
        ...
        for (const f of factures) {
            ...
            if (String(f.statut) === 'payee')
                points[idx].encaissements += total;
        }
        for (const a of achats) {
            const idx = bucketIndex(a.date, year, groupBy);
            if (idx === null)
                continue;
            const total = (a.lignes ?? []).reduce((sum, l) => {
                return sum + (Number(l.quantite) || 0) * (Number(l.prix_unitaire) || 0);
            }, 0);
            points[idx].achats += total;
            points[idx].depenses += total;
            if (String(a.statut) === 'payee' || Boolean(a.paye))
                points[idx].paiementsFournisseurs += total;
        }
